...
void TrieDemo()
{
    ...
    TrieBulkInsert(trie, nums, 6);

    /* Search for the existing strings. */
    assert(TrieHasExact(trie, "abcdefghijklmn") == true);
    assert(TrieHasExact(trie, "bcdefghijklmn") == false);

    /* Search for the strings matching the specified prefixes. */
    assert(TrieHasPrefixAs(trie, "A") == true);
    assert(TrieHasPrefixAs(trie, "BCD") == false);

    /* Remove the string in the trie. */
    TrieRemove(trie, nums[3]);

    /* Get the array of strings matching the specified prefix. */
    const char** strs;
    unsigned size;
    TrieGetPrefixAs(trie, "202", &strs, &size);
    assert(size == 5);
